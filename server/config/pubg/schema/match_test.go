...
func TestParseMatch(t *testing.T) {
	...
	assert.Equal(t, "official", match.MatchType)
	assert.Equal(t, "Summerland_Main", match.MapName)
	assert.Equal(t, false, match.IsCustomMatch)
	assert.Equal(t, "progress", match.SeasonState)
